...
Muralume 是一款注重隐私的原生视频播放器：它把 Mac 上的视频和文件夹变成播放列表，也能把任意播放队列变成动态桌面。应用直接使用本地文件，无需账号、云端上传或行为追踪。
...
可以添加单个视频、添加本机或外置磁盘中的文件夹，也可以直接从访达拖入。Muralume 会递归发现文件夹中的 MP4、MOV 和 M4V 视频、在本地生成缩略图，并支持按名称、创建时间或文件大小排序，无需重新整理磁盘文件。
...
1. 添加单个视频或文件夹，也可以从访达拖入播放器；拖入的第一个视频会立即播放。
