...
这段 20 秒演示从选择本地视频开始，将播放队列放到桌面文件后方，并在不影响桌面交互的情况下切换画面。
...
4. 按下 `⌘D` 应用当前动态桌面布局；使用 `⇧⌘D` 或“功能 → 自定义显示器…”选择同步播放或逐屏播放
...
同步模式会让已启用的显示器共享当前队列和同一播放时钟；逐屏模式则可让每块显示器独立循环一个指定视频。每块显示器都能单独选择桌面适配方式。显示器断开后，其分配的视频与适配设置会保留，并在同一显示器重连时恢复循环。连接、移除、重新排列显示器或更改分辨率时，同步队列不会重新启动。目前暂不支持把一个视频跨多块显示器拼接成单一画布。Muralume 使用 macOS 原生播放能力，桌面模式始终静音，也不阻止显示器休眠。使用电池或 UPS、开启低电量模式、温控压力升高或系统持续高负载时，会关闭装饰性的模糊填充；所有桌面窗口被遮挡、电池进入最终告警、温控达到严重级别，或正常睡眠与会话生命周期要求时，会暂停动态桌面，危急温控则暂停所有播放。
...
如需在登录后自动恢复当前队列，请在播放队列有效时打开设置并勾选“开机启动”。macOS 可能会要求你在“登录项”中批准 Muralume
...
- 多屏可选择同步播放，或让每块显示器独立循环一个视频，并分别设置适配方式、在断开重连后恢复分配；暂不支持跨屏拼接
