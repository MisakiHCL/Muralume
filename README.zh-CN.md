...
将当前播放队列置于每块已连接显示器的桌面文件和小组件之后，同时保持各个桌面完整可交互。连接、移除、重新排列显示器或更改分辨率时，播放队列不会重新启动。Muralume 使用 macOS 原生播放能力，桌面模式始终静音，也不阻止显示器休眠。使用电池或 UPS、开启低电量模式、温控压力升高或系统持续高负载时，会关闭装饰性的模糊填充；所有桌面窗口被遮挡、电池进入最终告警、温控达到严重级别，或正常睡眠与会话生命周期要求时，会暂停动态桌面，危急温控则暂停所有播放。
