...
Muralume 是一款私密、原生的 macOS 动态桌面应用，把你选择的视频和文件夹变成会动的桌面。内置的本地播放器、媒体库与自定义播放列表负责预览和整理，全程无需账号、云端上传或行为追踪。
...
| **按自己的方式整理** | 搜索媒体库，并创建“旅行”“天空”等有序、自定义命名的播放列表 |
...
2. 从访达打开支持的视频进行临时观看；若视频已在媒体库中，则直接从媒体库播放
3. 使用 `⌘F` 搜索并把视频整理到自定义命名的播放列表，再选择顺序播放、随机播放或循环当前视频，查看实际待播放队列
4. 按下 `⌘D` 应用当前动态桌面布局；使用 `⇧⌘D` 或“操作 → 自定义显示器…”选择同步播放或逐屏播放
...
可以从同一个“添加媒体”选择器中选择单个或多个视频、本机或外置磁盘中的文件夹，也可以混合选择或直接从访达拖入。Muralume 会递归发现文件夹中 AVFoundation 原生支持的 MP4、MOV、M4V、MPEG、MPEG-2、MPEG-TS、3GP、3G2、AVI 和 DV 视频，在本地生成缩略图，并支持按名称、创建时间或文件大小排序，无需重新整理磁盘文件。Muralume 运行期间会监听已授权文件夹，新增、删除和文件夹内重命名都会自动刷新；定期完整重扫会补偿外置磁盘、网络文件夹、云端占位文件或 FUSE 来源漏掉的变化，也可以点按“刷新媒体库”立即更新
...
媒体库是完整、持久且可排序的内容集合；播放队列呈现真实播放次序，包括随机顺序和从访达临时打开的项目。手动选择另一个媒体库视频时，会保留返回上一个视频的有效导航历史；刷新媒体库时，则会同步新增和确认删除的内容，不再丢弃当前队列

面对大型队列，界面先显示当前视频和有限数量的待播放项目，仅在需要时继续展开。媒体库仍通过原生虚拟列表承载完整内容浏览

### 搜索与自定义播放列表

按 `⌘F` 可按视频名称、来源、文件夹或相对路径搜索，且不会改变媒体库原始顺序。你可以创建“旅行”“天空”等自定义命名的播放列表，从媒体库添加视频、拖拽排序、重命名或删除分组，也能在列表内搜索。点击筛选结果时，仍会从该视频开始播放完整列表。磁盘离线时条目会保留最后已知名称；文件发生明确的重命名或移动后，会通过稳定文件身份重新关联

### 专注的原生播放体验

支持进度跳转、音量、倍速、全屏、顺序播放、随机播放和循环当前视频。随机模式会在新一轮开始前完整播放每个可用视频。队列中只有一个视频时，会直接将现有播放项跳回开头高效循环，同时禁用手动“上一个视频”和“下一个视频”。观看时控件自然隐去，常用的全局播放偏好也会在下次启动时恢复。
...
如需在登录后自动恢复当前队列，请在播放队列有效时打开设置并勾选“登录时启动”。macOS 可能会要求你在“登录项”中批准 Muralume
